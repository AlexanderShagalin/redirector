...
void do_request(const std::string& host, const std::string& port, const std::string& username)
{
    ...
    IoC::Resolve<void>("Server.Auth.Set", std::string(res[http::field::authentication_results]), username);
...
RegisterUserCommand::RegisterUserCommand(std::string username) :
    ICommand(), m_username(username) {}
